...
cred=credentials.Certificate('C:/Users/pc/Downloads/bci-database-7509c-firebase-adminsdk-bowkc-b9eaec0794.json')
firebase_admin.initialize_app(cred,{'databaseURL':'https://bci-database-7509c-default-rtdb.firebaseio.com'})
class LiveAdvance():
    """
    ...
    def on_new_com_data(self, *args, **kwargs):
        ...
        if data['action']=='push':
            ...
            data['action']=0
        print(data['action'])
        ref=db.reference('/BCI')
        BCI={'action':data['action'],
             'time':data['time'],
             'power':data['power']}
        ref.set(BCI)
        def callback(event):
...
def main():
    ...
    your_app_client_id = 'kxOSDsbeGVQvQ2s63djcg9SNQrCQOL4NCj9Qat2L'
    your_app_client_secret = 'E9YmuHVLxtUBuGAd0YRJvYXyh9zF5LI5EXiMf6lDjTt4pgdwVP7MbWclQD0m9oDXXY8IdlLt1jzq2mm8nxAQ65o0gtH9HOpq1M70W5KjwaZFhgi9FfHb5oCjghEst3YB'

    # Init live advance
    l = LiveAdvance(your_app_client_id, your_app_client_secret)

    trained_profile_name = 'hossamashor' # Please set a trained profile name here
